class Solution:
    def __init__(self, short_description: str, long_description: str, search_terms: list = None):
    ...
    def to_html(self, table = False):
        ...
        if table:
            result += (f"<h4>Long Description</h4><table style='width:100%'>" +
                       ...
                       "".join([f"<tr><td align='left'>{x[0] + 1}</td><td text-align='left'><p>{x[1]}</p></td></tr>" for x in
